...
sendPhoto(event){
  let { file,filename,filetype,songName,albumName,description,sheeturl } = this.state
  ...
  html2canvas(this.state.element[0]).then(canvas => {
    ...
    sendToback({file,filename,filetype,songName,albumName,description,sheeturl}).then(response => {
